...
session_start();
$day1 = date("Y-m-d ",mktime(0, 0 , 0,date("m"),date("d")-date("w")+1,date("Y")));
$day2 = date("Y-m-d ",mktime(23,59,59,date("m"),date("d")-date("w")+7,date("Y")));
...
//获取day1的数据
$result =$link->query("select* from booking,category where postdate between '{$day1}'and'{$day2}' and category.id =booking.cid and aid like'{$id}'");
$result1 =$link->query("select sum(amount) from booking where postdate between '{$day1}'and'{$day2}' and type like'-1' and aid like'{$id}'");
$result2 =$link->query("select sum(amount) from booking where postdate between '{$day1}'and'{$day2}' and type like'1' and aid like'{$id}'");
foreach ($result1 as $key => $value) {
	foreach ($value as $k => $v) {
			$arr[$k] = $v;
			
				  }
            $sum_out = $arr['sum(amount)'];
...
//遍历数据库输出明细
        foreach ($result as $key => $value) {
                    ...
                    echo "<li class='mui-table-view-cell'><a class='mui-navigate-right' href='home_details.php?id={$arr['id1']}'><div style='float:left' class='z'>{$money}</div><div style='float:right;margin-right:150px;'>{$arr['title']}</div></div><br><div class='p'>{$arr['postdate']}</div></a></li>";
                    ?>
                    <?php }?>
